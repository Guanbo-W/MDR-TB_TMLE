...
# Scenario 3: D is dependent of V and effect modification exsit
  ...
  d1_bar <- rbinom(clusterN,1,ilogit(1+2*v1_bar))
  d2_bar <- rbinom(clusterN,1,ilogit(0.5+1.5*v1_bar))
  d3_bar <- rbinom(clusterN,1,ilogit(1.5+0.3*v1_bar))
  D <- data.frame(d1_bar,d2_bar,d3_bar)
  D <- D [sum(d1_bar+d2_bar+d3_bar)!=0,]
  ...
  y0<-rbinom(N,1,ilogit(-2+3.5*v1+0.3*w1-0.1*u
                        +2.2*a1*(1-r1)+0.12*a2*(1-r2)+0.05*a3*(1-r3)+4.5*a1*u+4.1*a2*u+4.15*a3*u))
